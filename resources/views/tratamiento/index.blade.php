    ...
    <div class="bg-white rounded-2xl shadow-sm p-6 border border-gray-100 mb-6">
        <form method="GET" action="{{ route('tratamiento.index') }}">
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-4 items-end">
                ...
                <div>
                    <label class="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-2">Diagnóstico / Animal</label>
                    <select name="diagnostico_id" id="filtroAnimal"
                            class="w-full px-4 py-2.5 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-ganaderasoft-celeste focus:border-transparent transition-all">
                        ...
                        @foreach($diagnosticos as $diag)
                            @php
                                ...
                                $anNm= data_get($diag, 'animal.Nombre') ?? data_get($diag, 'animal.nombre') ?? ('Animal #'.$anId);
                                ...
                                $rNm = data_get($diag, 'animal.rebano.Nombre') ?? data_get($diag, 'animal.rebano.nombre') ?? '';
                                $fId = data_get($diag, 'animal.rebano.finca_id') ?? data_get($diag, 'animal.rebano.id_Finca') ?? '';
                                $fNm = data_get($diag, 'animal.rebano.finca.Nombre') ?? data_get($diag, 'animal.rebano.finca.nombre') ?? ('Finca #'.$fId);
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Diagnóstico</th>
                            <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Plan</th>
                            <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Fecha inicio</th>
                            <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Fecha fin</th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        @php
                            ...
                            $animalId = data_get($tratamiento, 'diagnostico.animal_id') ?? data_get($tratamiento, 'diagnostico.fk_etapa_animal_anid') ?? data_get($tratamiento, 'diagnostico.etapa_animal.animal_id') ?? '';
                            $animalRefId = data_get($tratamiento, 'diagnostico.animal.id') ?? data_get($tratamiento, 'diagnostico.animal.id_Animal') ?? data_get($tratamiento, 'diagnostico.etapa_animal.animal.id') ?? $animalId;
                            $animalNombre = data_get($tratamiento, 'diagnostico.animal.Nombre') ?? data_get($tratamiento, 'diagnostico.animal.nombre') ?? data_get($tratamiento, 'diagnostico.etapa_animal.animal.nombre') ?? data_get($tratamiento, 'diagnostico.etapa_animal.animal.Nombre') ?? ($animalId ? 'Animal #'.$animalId : 'N/A');
                            
                            $sexoVal = data_get($tratamiento, 'diagnostico.animal.sexo') ?? data_get($tratamiento, 'diagnostico.animal.Sexo') ?? data_get($tratamiento, 'diagnostico.etapa_animal.animal.sexo') ?? data_get($tratamiento, 'diagnostico.etapa_animal.animal.Sexo') ?? 'H';
                                ...
                                <div class="flex justify-center space-x-2">
                                    ...
                                    <form method="POST" action="{{ route('tratamiento.destroy', $tId) }}" class="inline-block" id="form-delete-tratamiento-{{ $tId }}">
                                        ...
                                        <button type="button" onclick="openGenericConfirmModal({
                                            ...
                                            intent: 'danger',
                                            title: 'Eliminar registro de tratamiento',
