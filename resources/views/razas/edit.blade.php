...
@section('title', 'Editar ' . $catalog['name'])
...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div class="flex items-center space-x-4">
            ...
            <div>
                ...
                <p class="text-gray-500 text-sm mt-1">Actualizar registro en {{ $catalog['name'] }}</p>
            </div>
        </div>
        <div>
    ...
    <form method="POST" action="{{ route($catalog['slug'].'.update', $item['id']) }}" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                <!-- Card 1: Identificación y Ubicación -->
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-6">
                    <h3 class="text-xl font-bold text-ganaderasoft-negro border-b border-gray-100 pb-3 flex items-center gap-2">
                        <span>🏷️</span> Identificación y ubicación
                    </h3>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="text" id="siglas" name="siglas" value="{{ old('siglas', $item['siglas'] ?? '') }}" maxlength="6"
                                   class="w-full px-4 py-3 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-ganaderasoft-celeste focus:border-transparent transition-all">
                            @error('siglas')<p class="text-xs text-red-600 font-medium mt-1">{{ $message }}</p>@enderror
                        </div>

                        <div class="md:col-span-2">
                            <label for="finca_id" class="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-2">Finca <span class="text-red-500">*</span></label>
                            <select id="finca_id" name="finca_id" required
                                ...
                                <option value="" disabled {{ old('finca_id', $item['finca_id'] ?? '') == '' ? 'selected' : '' }}>Seleccione una finca...</option>
                                @foreach($fincas as $finca)
                                    @php
                                        $fId = is_array($finca) ? ($finca['id'] ?? $finca['finca_id'] ?? '') : $finca->id;
                                        $fNombre = is_array($finca) ? ($finca['nombre'] ?? '') : $finca->nombre;
                                    @endphp
                                    <option value="{{ $fId }}" {{ old('finca_id', $item['finca_id'] ?? '') == $fId ? 'selected' : '' }}>
                                        {{ $fNombre }}
                                    </option>
                                @endforeach
                            </select>
                ...
                <!-- Card 2: Clasificación Productiva -->
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-6">
                    <h3 class="text-xl font-bold text-ganaderasoft-negro border-b border-gray-100 pb-3 flex items-center gap-2">
                        <span>📊</span> Clasificación productiva
                    </h3>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <select id="tipo_animal_id" name="tipo_animal_id" required
                                ...
                                @foreach($tiposAnimal as $tipo)
                                    @php
                                        ...
                                        $isSelected = old('tipo_animal_id', $item['tipo_animal_id'] ?? '') == $tId;
                                    @endphp
                        ...
                        <div>
                            ...
                            <select id="tipo_raza" name="tipo_raza"
                                ...
                                @php $currentTipoRaza = old('tipo_raza', $item['tipo_raza'] ?? ''); @endphp
                                <option value="Pura" {{ $currentTipoRaza == 'Pura' ? 'selected' : '' }}>Pura</option>
                                <option value="Cruzada" {{ $currentTipoRaza == 'Cruzada' ? 'selected' : '' }}>Cruzada</option>
                                <option value="Sintética" {{ $currentTipoRaza == 'Sintética' ? 'selected' : '' }}>Sintética</option>
                            </select>
                        ...
                        <div>
                            ...
                            <select id="proporcion_raza" name="proporcion_raza"
                                    class="w-full px-4 py-3 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-ganaderasoft-celeste focus:border-transparent transition-all bg-white">
                                <option value="" disabled>Seleccione...</option>
                                @php $currentProporcion = old('proporcion_raza', $item['proporcion_raza'] ?? ''); @endphp
                                <option value="Grande" {{ $currentProporcion == 'Grande' ? 'selected' : '' }}>Grande</option>
                                <option value="Mediano" {{ $currentProporcion == 'Mediano' ? 'selected' : '' }}>Mediano</option>
                                <option value="Pequeño" {{ $currentProporcion == 'Pequeño' ? 'selected' : '' }}>Pequeño</option>
                            </select>
                ...
                <!-- Card 3: Características Físicas y Origen -->
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-6">
                    <h3 class="text-xl font-bold text-ganaderasoft-negro border-b border-gray-100 pb-3 flex items-center gap-2">
                        <span>🧬</span> Características y origen
                    </h3>
                    
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label for="pelaje" class="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-2">Pelaje típico</label>
                            <input type="text" id="pelaje" name="pelaje" value="{{ old('pelaje', $item['pelaje'] ?? '') }}" maxlength="80"
                                   class="w-full px-4 py-3 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-ganaderasoft-celeste focus:border-transparent transition-all">
                        ...
                        <div>
                            ...
                            <input type="text" id="origen" name="origen" value="{{ old('origen', $item['origen'] ?? '') }}" maxlength="60"
                                   class="w-full px-4 py-3 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-ganaderasoft-celeste focus:border-transparent transition-all">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea id="caracteristica_especial" name="caracteristica_especial" rows="2" maxlength="80"
                                      class="w-full px-4 py-3 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-ganaderasoft-celeste focus:border-transparent transition-all">{{ old('caracteristica_especial', $item['caracteristica_especial'] ?? '') }}</textarea>
                    ...
                    <div class="p-6 space-y-5">
                        ...
                        <div class="space-y-3 text-xs text-gray-600 border-b border-gray-100 pb-4">
                            <div class="flex justify-between">
                                ...
                                <span id="previewSiglas" class="text-right">{{ $item['siglas'] ?? '-' }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="font-semibold">Tipo de animal:</span>
                                <span id="previewTipoAnimal" class="text-right">{{ $item['tipo_animal']['nombre'] ?? 'Seleccione...' }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="font-semibold">Propósito:</span>
                                <span id="previewProposito" class="text-right">{{ $item['proposito'] ?? 'Seleccione...' }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="font-semibold">Tipo de raza:</span>
                                <span id="previewTipoRaza" class="text-right">{{ $item['tipo_raza'] ?? '-' }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="font-semibold">Visibilidad:</span>
                                <span class="text-right font-medium {{ empty($item['finca_id']) ? 'text-blue-600' : 'text-ganaderasoft-azul' }}">
                                    {{ empty($item['finca_id']) ? '🌐 Pública' : '🔒 Privada' }}
                                </span>
                            </div>
                        </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    siglasInput?.addEventListener('input', updatePreview);
    tipoAnimalInput?.addEventListener('change', updatePreview);
    propositoInput?.addEventListener('change', updatePreview);
    tipoRazaInput?.addEventListener('change', updatePreview);
