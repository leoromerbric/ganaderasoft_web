...
@section('title', 'Reproducción animal')

@section('content')
@php
    $totalRegistros = count($reproducciones);
    $totalIA = count(array_filter($reproducciones, function($r) {
        $t = strtolower((string)($r['tipo'] ?? $r['tipo_reproduccion'] ?? $r['repro_tipo_reproduccion'] ?? ''));
        return str_contains($t, 'ia') || str_contains($t, 'inseminacion') || str_contains($t, 'inseminación') || str_contains($t, 'artificial');
    }));
    $totalMonta = count(array_filter($reproducciones, function($r) {
        $t = strtolower((string)($r['tipo'] ?? $r['tipo_reproduccion'] ?? $r['repro_tipo_reproduccion'] ?? ''));
        return str_contains($t, 'monta') || str_contains($t, 'natural') || str_contains($t, 'directa');
    }));
    $currentMonth = date('Y-m');
    $esteMes = count(array_filter($reproducciones, function($r) use ($currentMonth) {
        $f = $r['fecha'] ?? $r['fecha_reproduccion'] ?? $r['repro_fecha_reproduccion'] ?? '';
...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div class="flex items-center space-x-4">
            ...
            <div>
                ...
                <p class="text-gray-500 text-sm mt-1">Gestión de eventos reproductivos, inseminaciones, montas y seguimiento ginecológico</p>
            </div>
        </div>
        <div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        <div class="bg-white rounded-2xl shadow-sm p-6 border border-gray-100 flex items-center justify-between">
            <div>
                <p class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Total eventos</p>
        ...
        <div class="bg-white rounded-2xl shadow-sm p-6 border border-gray-100 flex items-center justify-between">
            <div>
                <p class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Eventos este mes</p>
    ...
    <div class="bg-white rounded-2xl shadow-sm p-6 border border-gray-100">
        <div class="space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
                ...
                <div>
                    <label class="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-2">Tipo de evento</label>
                    <select id="filtroTipo"
                        ...
                        <option value="palpacion">Palpación / Chequeo</option>
                    </select>
                </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm overflow-hidden border border-gray-100" id="tableContainer">
        @if(count($reproducciones) > 0)
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200" id="tablaReproducciones">
                    <thead class="bg-gray-50/80">
                        <tr>
                            ...
                            <th class="px-6 py-3.5 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Tipo de evento</th>
                            <th class="px-6 py-3.5 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Fecha del evento</th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-100 text-sm">
                        ...
                        @php
                            $rId = $reproduccion['id'] ?? $reproduccion['repro_id'] ?? null;
                            ...
                            $fincaId = (string)(data_get($reproduccion, 'etapa_animal.animal.rebano.finca.id') ?? data_get($reproduccion, 'animal.rebano.finca.id') ?? data_get($reproduccion, 'animal.rebano.finca_id') ?? '');
                            ...
                            $rebanoId = (string)(data_get($reproduccion, 'etapa_animal.animal.rebano.id') ?? data_get($reproduccion, 'animal.rebano.id') ?? data_get($reproduccion, 'animal.rebano_id') ?? '');
                            ...
                            $tipoEvento = $reproduccion['tipo'] ?? $reproduccion['tipo_reproduccion'] ?? $reproduccion['repro_tipo_reproduccion'] ?? 'Evento';
                            ...
                            $tipoLower = strtolower((string)$tipoEvento);
                            ...
                            $isArchivado = (bool)(data_get($reproduccion, 'animal.archivado') ?? data_get($reproduccion, 'animal.is_archivado') ?? false);
                        ...
                        @endphp
                        <tr class="hover:bg-gray-50/80 transition-colors fila-reproduccion" 
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                @else
                                    <span class="inline-flex items-center px-2.5 py-1 rounded-md text-xs font-bold bg-gray-50 text-gray-700 border border-gray-200">
                                        🔬 {{ $tipoEvento }}
                                    </span>
                                @endif
                            </td>
                            ...
                            <td class="px-6 py-4 text-sm text-gray-600 max-w-xs truncate">
                                @if($observacion)
                                    <span title="{{ $observacion }}">{{ $observacion }}</span>
                                @else
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <div class="flex justify-center items-center gap-2">
                                    <!-- Botón Ver Detalles -->
                                    <a href="{{ route('reproduccion-animal.show', $rId) }}"
                                    ...
                                    <!-- Botón Editar -->
                                    <a href="{{ route('reproduccion-animal.edit', $rId) }}"
                                    ...
                                    <form method="POST" action="{{ route('reproduccion-animal.destroy', $rId) }}" class="inline-block" id="form-delete-repro-{{ $rId }}">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" onclick="openGenericConfirmModal({
                                            formId: 'form-delete-repro-{{ $rId }}',
                                            intent: 'danger',
                                            title: 'Eliminar registro reproductivo',
                                            message: '¿Estás seguro de que deseas eliminar este registro reproductivo animal? Esta acción no se puede revertir.',
        ...
        @else
            <div class="p-12 text-center space-y-4">
                ...
                <div class="space-y-1">
                    <h3 class="text-lg font-bold text-gray-900">No hay registros reproductivos guardados</h3>
                    <p class="text-sm text-gray-500 max-w-md mx-auto">Comienza registrando eventos de reproducción animal para mantener un control ginecológico y de fertilidad completo.</p>
                </div>
                <div class="pt-2">
                    <a href="{{ route('reproduccion-animal.create') }}"
                       class="px-6 py-3 bg-ganaderasoft-verde-oscuro text-white rounded-xl hover:bg-opacity-90 transition-all font-semibold text-sm shadow-md hover:shadow-lg inline-flex items-center gap-2">
                        <span>+</span> Registrar nueva reproducción
                    </a>
                </div>
            </div>
        @endif
    ...
    <div id="emptyFilteredState" class="hidden bg-white rounded-2xl shadow-sm border border-gray-100 p-12 text-center space-y-3">
        ...
        <div class="space-y-1">
            <h4 class="text-base font-bold text-gray-900">No se encontraron eventos reproductivos</h4>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function aplicarFiltros() {
        ...
        document.querySelectorAll('.fila-reproduccion').forEach(function(row) {
            ...
            if (tipo) {
                if (tipo === 'ia' && !(rowTipo.includes('ia') || rowTipo.includes('inseminaci') || rowTipo.includes('artific'))) {
                    ...
                    matchTipo = false;
                } else if (tipo === 'parto' && !rowTipo.includes('parto')) {
                    matchTipo = false;
                } else if (tipo === 'aborto' && !rowTipo.includes('aborto')) {
                    matchTipo = false;
                } else if (tipo === 'palpacion' && !(rowTipo.includes('palpaci') || rowTipo.includes('chequeo') || rowTipo.includes('gestaci'))) {
                    matchTipo = false;
                }
            }
        ...
        if (emptyFiltered) {
            const totalRows = document.querySelectorAll('.fila-reproduccion').length;
