...
@php
    ...
    $nombreFinca = $finca['nombre'] ?? $finca['Nombre'] ?? 'Finca';
...
<div class="space-y-8">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <!-- Rebaños -->
        <a href="{{ route('rebanos.index') }}" class="group bg-white rounded-2xl shadow-sm p-6 hover:shadow-md transition-all duration-200 border-t-4 border-ganaderasoft-azul">
            ...
            <p class="text-xs text-gray-500 mb-4">Gestión de agrupaciones y lotes de animales</p>
            <div class="flex items-center text-ganaderasoft-azul font-semibold text-sm group-hover:text-ganaderasoft-celeste transition-colors">
        ...
        <!-- Lista de Animales -->
        <a href="{{ route('animales.index') }}" class="group bg-white rounded-2xl shadow-sm p-6 hover:shadow-md transition-all duration-200 border-t-4 border-ganaderasoft-celeste">
            <div class="flex items-center justify-between mb-4">
            ...
            <h3 class="text-xl font-bold text-ganaderasoft-negro mb-1">Lista de Animales</h3>
            <p class="text-xs text-gray-500 mb-4">Registro y fichas individuales del rebaño</p>
            <div class="flex items-center text-ganaderasoft-celeste font-semibold text-sm group-hover:text-ganaderasoft-azul transition-colors">
        ...
        <!-- Personal de Finca -->
        <a href="{{ route('personal-finca.index') }}" class="group bg-white rounded-2xl shadow-sm p-6 hover:shadow-md transition-all duration-200 border-t-4 border-ganaderasoft-verde-oscuro">
            <div class="flex items-center justify-between mb-4">
            ...
            <p class="text-xs text-gray-500 mb-4">Trabajadores, encargados y técnicos asignados</p>
            <div class="flex items-center text-ganaderasoft-verde-oscuro font-semibold text-sm group-hover:text-ganaderasoft-verde transition-colors">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div class="bg-gray-50/70 p-4 rounded-xl border border-gray-100">
                ...
                <p class="font-bold text-ganaderasoft-negro">
                    {{ isset($terreno['superficie']) || isset($terreno['Superficie']) ? ($terreno['superficie'] ?? $terreno['Superficie']).' ha' : 'No registrada' }}
                </p>
            </div>

            <div class="bg-gray-50/70 p-4 rounded-xl border border-gray-100">
                <p class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Tipo de Relieve</p>
                <p class="font-bold text-ganaderasoft-negro">{{ $terreno['relieve'] ?? $terreno['Relieve'] ?? 'No especificado' }}</p>
            </div>

            <div class="bg-gray-50/70 p-4 rounded-xl border border-gray-100">
                <p class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Textura del Suelo</p>
                <p class="font-bold text-ganaderasoft-negro">{{ $terreno['suelo_textura'] ?? $terreno['Suelo_Textura'] ?? 'No especificado' }}</p>
            </div>

            <div class="bg-gray-50/70 p-4 rounded-xl border border-gray-100">
                <p class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">pH del Suelo</p>
                <p class="font-bold text-ganaderasoft-negro">{{ $terreno['ph_suelo'] ?? $terreno['ph_Suelo'] ?? 'No especificado' }}</p>
            </div>

            <div class="bg-gray-50/70 p-4 rounded-xl border border-gray-100">
                <p class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Fuente de Agua & Riego</p>
                <p class="font-bold text-ganaderasoft-negro">
                    {{ $terreno['fuente_agua'] ?? $terreno['Fuente_Agua'] ?? 'No especificado' }}
                </p>
                <p class="text-xs text-gray-500 mt-1">Método: {{ $terreno['riego_metodo'] ?? $terreno['Riego_Metodo'] ?? '-' }}</p>
            </div>
        </div>
    </div>
</div>
