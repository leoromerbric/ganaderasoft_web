...
@section('content')
    <div>
        <!-- Page Title -->
        <div class="mb-8">
            <h2 class="text-3xl font-bold text-ganaderasoft-negro">Editar Finca</h2>
            <p class="text-gray-600 mt-1">Modifique la información de la finca</p>
        </div>
        ...
        <!-- Form -->
        <form method="POST" action="{{ route('fincas.update', $finca['id_Finca']) }}" class="bg-white rounded-xl shadow-md p-6">
            @csrf
            @method('PUT')
                        ...
                        <select name="Explotacion_Tipo" id="Explotacion_Tipo" required
                            ...
                            @foreach($tipoExplotacion as $tipo)
                                <option value="{{ $tipo['nombre'] }}" 
                                    {{ old('Explotacion_Tipo', $finca['Explotacion_Tipo']) == $tipo['nombre'] ? 'selected' : '' }}>
                                    {{ $tipo['nombre'] }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    <div>
                        ...
                        <input type="number" name="Superficie" id="Superficie" step="0.01" required
                            value="{{ old('Superficie', $finca['terreno']['Superficie'] ?? '') }}"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-ganaderasoft-celeste focus:border-transparent">
                    </div>
                    ...
                    <div>
                        ...
                        <select name="Relieve" id="Relieve" required
                            ...
                            @foreach($tipoRelieve as $relieve)
                                <option value="{{ $relieve['valor'] }}" 
                                    {{ old('Relieve', $finca['terreno']['Relieve'] ?? '') == $relieve['valor'] ? 'selected' : '' }}>
                                    {{ $relieve['valor'] }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="Suelo_Textura" id="Suelo_Textura" required
                            ...
                            @foreach($texturaSuelo as $textura)
                                <option value="{{ $textura['nombre'] }}" 
                                    {{ old('Suelo_Textura', $finca['terreno']['Suelo_Textura'] ?? '') == $textura['nombre'] ? 'selected' : '' }}>
                                    {{ $textura['nombre'] }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="ph_Suelo" id="ph_Suelo" required
                            ...
                            @foreach($phSuelo as $ph)
                                <option value="{{ $ph['codigo'] }}" 
                                    {{ old('ph_Suelo', $finca['terreno']['ph_Suelo'] ?? '') == $ph['codigo'] ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <input type="number" name="Precipitacion" id="Precipitacion" step="0.01" required
                            value="{{ old('Precipitacion', $finca['terreno']['Precipitacion'] ?? '') }}"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-ganaderasoft-celeste focus:border-transparent">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" name="Velocidad_Viento" id="Velocidad_Viento" step="0.01" required
                            value="{{ old('Velocidad_Viento', $finca['terreno']['Velocidad_Viento'] ?? '') }}"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-ganaderasoft-celeste focus:border-transparent">
                    </div>
                </div>
            </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    <div>
                        ...
                        <input type="text" name="Temp_Anual" id="Temp_Anual" required
                            value="{{ old('Temp_Anual', $finca['terreno']['Temp_Anual'] ?? '') }}"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-ganaderasoft-celeste focus:border-transparent">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="Temp_Min" id="Temp_Min" required
                            value="{{ old('Temp_Min', $finca['terreno']['Temp_Min'] ?? '') }}"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-ganaderasoft-celeste focus:border-transparent">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="Temp_Max" id="Temp_Max" required
                            value="{{ old('Temp_Max', $finca['terreno']['Temp_Max'] ?? '') }}"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-ganaderasoft-celeste focus:border-transparent">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" name="Radiacion" id="Radiacion" step="0.01" required
                            value="{{ old('Radiacion', $finca['terreno']['Radiacion'] ?? '') }}"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-ganaderasoft-celeste focus:border-transparent">
                    </div>
                </div>
            </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <select name="Fuente_Agua" id="Fuente_Agua" required
                            ...
                            @foreach($fuenteAgua as $fuente)
                                <option value="{{ $fuente['nombre'] }}" 
                                    {{ old('Fuente_Agua', $finca['terreno']['Fuente_Agua'] ?? '') == $fuente['nombre'] ? 'selected' : '' }}>
                                    {{ $fuente['nombre'] }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="number" name="Caudal_Disponible" id="Caudal_Disponible" required
                            value="{{ old('Caudal_Disponible', $finca['terreno']['Caudal_Disponible'] ?? '') }}"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-ganaderasoft-celeste focus:border-transparent">
                    </div>
                        ...
                        <select name="Riego_Metodo" id="Riego_Metodo" required
                            ...
                            @foreach($metodoRiego as $metodo)
                                <option value="{{ $metodo['nombre'] }}" 
                                    {{ old('Riego_Metodo', $finca['terreno']['Riego_Metodo'] ?? '') == $metodo['nombre'] ? 'selected' : '' }}>
                                    {{ $metodo['nombre'] }}
                                </option>
                            @endforeach
                        </select>
            ...
            <div class="flex justify-end space-x-4 mt-8 pt-6 border-t border-gray-200">
                ...
                <button type="submit" 
                    class="px-6 py-2 bg-ganaderasoft-verde-oscuro hover:bg-opacity-90 text-white rounded-lg font-medium transition-colors duration-200 shadow-sm">
                    Actualizar
                </button>
            </div>
        </form>
