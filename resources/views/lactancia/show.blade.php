...
@php
    ...
    $animalNombre = data_get($lactancia, 'animal.nombre')
        ?? ('Animal #'.(data_get($lactancia, 'animal_id') ?? 'N/A'));
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-4">
                ...
                <div class="p-5 bg-pink-50/60 border border-pink-100 rounded-2xl flex items-center justify-between">
                    <div class="flex items-center space-x-4">
                        <div class="w-14 h-14 rounded-2xl bg-white border border-pink-200 text-pink-700 font-bold flex items-center justify-center text-3xl shadow-xs">
                            🐄
                        </div>
                        <div>
                            <p class="text-xl font-extrabold text-gray-900">{{ $animalNombre }}</p>
                            @if($animalCodigo)
                                <p class="text-xs text-gray-500 font-mono mt-0.5">Código: #{{ $animalCodigo }}</p>
                            @endif
                            <p class="text-xs text-gray-600 font-semibold mt-1">Etapa: {{ $etapaNombre }}</p>
                        </div>
                    </div>

                    <div>
                        <span class="px-3.5 py-1.5 text-xs font-bold rounded-full bg-pink-100 text-pink-800 border border-pink-200">
                            ♀️ Hembra
                        </span>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div class="p-5 bg-blue-50/60 border border-blue-100 rounded-2xl space-y-2">
                        ...
                        <p class="text-lg font-bold text-gray-900">
                            ...
                            @else
                                <span class="text-emerald-600 font-bold">En curso</span>
                            @endif
                        </p>
                    </div>
                    ...
                    <div class="p-5 bg-amber-50/60 border border-amber-100 rounded-2xl space-y-2">
                        ...
                        <p class="text-lg font-bold text-gray-900">
                            ...
                            @else
                                <span class="text-gray-400 font-medium">No registrada</span>
                            @endif
                        </p>
                    </div>
                </div>
