    ...
    <div class="bg-white rounded-2xl shadow-sm p-6 border border-gray-100 mb-6">
        <form method="GET" action="{{ route('semen-toro.index') }}">
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 items-end">
                ...
                <div>
                    <label class="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-2">Toro</label>
                    <select name="toro_id" id="filtroAnimal"
                            class="w-full px-4 py-2.5 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-ganaderasoft-celeste focus:border-transparent transition-all">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            <th class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Toro</th>
                        ...
                        <tr class="hover:bg-gray-50 transition-colors" data-animal-id="{{ $semen['animal_id'] ?? '' }}">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center space-x-3">
                                    <div class="w-10 h-10 shrink-0 rounded-xl bg-blue-50 text-blue-600 border border-blue-100 flex items-center justify-center font-bold text-lg">
                                        🐂
                                    </div>
                                    <div class="overflow-hidden">
                                        <p class="font-bold text-gray-900 truncate">{{ $semen['toro']['nombre'] ?? $semen['toro']['Nombre'] ?? ('Toro #'.($semen['animal_id'] ?? $semen['id_Toro'] ?? 'N/A')) }}</p>
                                        <p class="text-xs text-gray-400">ID: #{{ $semen['animal_id'] ?? '' }}</p>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if(isset($semen['estado']))
                                    <span class="px-3 py-1 text-xs font-semibold rounded-full {{ $semen['estado'] ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-red-50 text-red-700 border border-red-200' }}">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center text-sm">
                                <div class="flex justify-center space-x-2">
                                    ...
                                    <form method="POST" action="{{ route('semen-toro.destroy', $semen['id']) }}" class="inline-block" id="form-delete-semen-{{ $semen['id'] }}">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" onclick="openGenericConfirmModal({
                                            formId: 'form-delete-semen-{{ $semen['id'] }}',
                                            intent: 'danger',
                                            title: 'Eliminar registro de semen',
                                            message: '¿Estás seguro de que deseas eliminar este registro de semen de toro? Esta acción no se puede deshacer.',
