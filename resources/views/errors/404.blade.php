...
<!-- Contenedor Principal a Pantalla Completa sin Sidebar -->
<div class="min-h-screen bg-slate-50 text-gray-800 flex flex-col justify-between selection:bg-ganaderasoft-celeste selection:text-white relative overflow-hidden">
    ...
    @if($isAuthenticated)
        {{-- Navbar oficial del sistema para usuarios autenticados (Avatar, Perfil, Rol, etc.) --}}
        <div class="relative z-30">
            <x-layouts.navbar />
        </div>
    @else
        {{-- Header público para usuarios invitados --}}
        <x-layouts.public-header :showHomeLink="true" />
    @endif

    <!-- CONTENIDO CENTRAL 404 (A Pantalla Completa y Centrado) -->
    <main class="flex-grow flex items-center justify-center relative z-10 py-8 sm:py-14 px-4 sm:px-6 lg:px-8">
        <div class="max-w-xl w-full mx-auto text-center space-y-4 sm:space-y-6">
            ...
            <div class="select-none leading-none my-1 sm:my-2">
                <span class="font-black tracking-tight text-ganaderasoft-azul leading-none block drop-shadow-xs" style="font-size: clamp(4.5rem, 14vw, 8.5rem); line-height: 1;">
                    404
                </span>
            </div>

            <!-- Título y Mensaje con Tipografía Fluida -->
            <div class="space-y-2 max-w-xl mx-auto px-2">
                <h1 class="font-extrabold text-ganaderasoft-negro tracking-tight" style="font-size: clamp(1.35rem, 4.5vw, 2.75rem); line-height: 1.2;">
                    @if($isAuthenticated)
                ...
                <p class="text-xs sm:text-base lg:text-lg text-gray-600 font-normal leading-relaxed">
                    @if($isAuthenticated)
