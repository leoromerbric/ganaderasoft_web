...
@section('title', 'Registrar servicio reproductivo')
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div class="flex items-center space-x-4">
            ...
            <div>
                ...
                <p class="text-gray-500 text-sm mt-1">Registra una inseminación artificial o monta natural para el seguimiento reproductivo</p>
            </div>
        </div>
        <div>
            ...
            <div class="lg:col-span-2 flex flex-col space-y-6">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-6">
                    <h3 class="text-xl font-bold text-ganaderasoft-negro border-b border-gray-100 pb-3 flex items-center gap-2">
                        <span>🐄</span> Selección de la hembra receptora
                    </h3>
                    ...
                    <div>
                        <label for="servicio_animal_id" class="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-2">
                            Hembra para el servicio <span class="text-red-500">*</span>
                        </label>
                        <select name="animal_id" id="servicio_animal_id" required
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select name="tipo" id="tipo" required
                                ...
                                <option value="TE" {{ old('tipo') == 'TE' ? 'selected' : '' }}>🔬 Transferencia de Embriones (TE)</option>
                            </select>
                        ...
                        <div>
                            ...
                            <select name="semen_id" id="semen_id"
                                ...
                                @foreach($semenToros as $semen)
                                    ...
                                    @endphp
                                    <option value="{{ $semId }}" data-nombre="{{ $toroNombre }}" {{ old('semen_id') == $semId ? 'selected' : '' }}>
                        ...
                        <!-- Registro de Celo Previo Asociado (Filtrado reactivo) -->
                            ...
                            <select name="celo_id" id="servicio_celo_id"
                                ...
                                @foreach($registrosCelo as $celo)
                                    @php
                                        ...
                                        $animalNombre = data_get($celo, 'animal.Nombre') ?? data_get($celo, 'animal.nombre') ?? '';
                                    @endphp
                                    <option value="{{ $celId }}"
                                            data-animal-id="{{ $celoAnimalId }}"
                                            {{ (string)old('celo_id', $presetCeloId ?? request('registro_celo_id')) === (string)$celId ? 'selected' : '' }}>
                                        🔥 Celo #{{ $celId }} — {{ $animalNombre }} ({{ $celoFecha ? date('d/m/Y', strtotime($celoFecha)) : 'S/F' }})
                                    </option>
                                @endforeach
                            </select>
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                    ...
                    <div class="p-6 space-y-5">
                        ...
                        <div class="p-4 bg-blue-50/70 border border-blue-100 rounded-2xl flex items-center space-x-3">
                            <div id="previewIcono" class="w-12 h-12 rounded-xl bg-white border border-blue-200 text-blue-700 font-bold flex items-center justify-center text-2xl shadow-xs shrink-0">
                                🐄
                            </div>
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 space-y-4">
                    ...
                    <div class="space-y-1.5 pt-1">
                        <span class="text-xs font-semibold text-gray-600 block uppercase tracking-wider">Manejo de pajuelas (IA):</span>
                        <ul class="list-disc list-inside space-y-1 text-xs text-gray-500 leading-relaxed">
                            <li>Descongelar en baño maría a <strong>35°C - 37°C</strong> por 30-40 segundos.</li>
                            <li>Secar con toalla de papel limpia antes de cortar.</li>
                            <li>Evitar choque térmico y depositar en el cuerpo del útero.</li>
                        </ul>
                    </div>
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', function () {
    const aSelect = document.getElementById('servicio_animal_id');
    ...
    // Base options for celo
    const baseCeloOptions = celoSelect ? Array.from(celoSelect.querySelectorAll('option[value]')).filter(o => o.value !== '') : [];
    ...
    function updateCeloOptions() {
        ...
        const selectedAnimalId = aSelect ? aSelect.value : '';
        const previousVal = celoSelect.value;

        celoSelect.innerHTML = '<option value="">-- Sin registro de celo previo vinculado --</option>';

        if (!selectedAnimalId) {
            return;
        }

        const filtered = baseCeloOptions.filter(o => o.dataset.animalId === selectedAnimalId);
        filtered.forEach(o => {
            celoSelect.appendChild(o.cloneNode(true));
        });

        if (Array.from(celoSelect.options).some(o => o.value === previousVal)) {
            celoSelect.value = previousVal;
        }
    }
    ...
    function updateAllPreviews() {
        ...
        // Semen
        const semOpt = semenSelect ? semenSelect.options[semenSelect.selectedIndex] : null;
        if (previewSemen) {
    ...
    if (semenSelect) semenSelect.addEventListener('change', updateAllPreviews);
