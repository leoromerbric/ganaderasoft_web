...
@section('content')
    <div class="space-y-8">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                <h1 class="text-3xl font-bold text-ganaderasoft-negro">Gestión de usuarios</h1>
                <p class="text-gray-500 text-sm mt-1">Administración de cuentas, roles y estados de acceso</p>
            </div>
                        ...
                        <tbody class="bg-white divide-y divide-gray-100 text-sm">
                            @foreach($users as $user)
                                ...
                                <tr class="hover:bg-gray-50/80 transition-colors fila-usuario flex justify-between items-center w-full"
                                    ...
                                    <td class="w-1/5 px-6 py-4 whitespace-nowrap overflow-hidden text-ellipsis">
                                        <div class="flex flex-wrap gap-1">
                                            @if(!empty($user['roles']))
                                                @foreach($user['roles'] as $r)
                                                    @php
                                                        $roleCode = is_array($r) ? ($r['code'] ?? '') : $r;
                                                        $roleName = is_array($r) ? ($r['name'] ?? ucfirst($roleCode)) : ucfirst($roleCode);
                                                    @endphp
                                                    <span class="px-3 py-1 text-xs font-semibold rounded-full border bg-gray-50 text-gray-700 border-gray-200">
                                                        {{ $roleName }}
                                                    </span>
                                                @endforeach
                                            @else
                                    ...
                                    <td class="w-1/5 px-6 py-4 whitespace-nowrap overflow-hidden text-ellipsis">
                                        @if($isSuspended)
                                            <span class="px-3 py-1 text-xs font-semibold rounded-full bg-red-50 text-red-700 border border-red-200">Suspendido</span>
                                        @else
                                    ...
                                    <td class="w-1/5 px-6 py-4 whitespace-nowrap text-center text-sm">
                                        <div class="flex justify-center space-x-2">
                                            ...
                                            <form method="POST" action="{{ route('admin.users.toggle-status', $user['id']) }}" class="inline-block" id="form-toggle-{{ $user['id'] }}">
                                                @csrf
    ...
    <script>
        ...
        function aplicarFiltros() {
            ...
            document.querySelectorAll('.fila-usuario').forEach(function(row) {
                ...
                row.style.display = (matchesNombre && matchesRole && matchesStatus) ? '' : 'none';
            });
        }
    </script>
@endsection
