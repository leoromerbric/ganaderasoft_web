            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            <label for="dias" class="block text-xs font-semibold text-gray-600 uppercase tracking-wider mb-2">Días de gestación <span class="text-red-500">*</span></label>
                            <input type="number" id="dias" name="dias" value="{{ old('dias', $item['dias'] ?? '') }}" required 
                            ...
                            @error('dias')<p class="text-xs text-red-600 font-medium mt-1">{{ $message }}</p>@enderror
                        </div>
                    </div>
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const previewDias = document.getElementById('previewDias');
    ...
    function updatePreview() {
        ...
        const diasVal = diasInput?.value.trim() || '';
        if (previewNombre) previewNombre.textContent = diasVal || 'Sin registro';
        if (previewIcono) previewIcono.textContent = diasVal ? diasVal.charAt(0).toUpperCase() : '#';

    }

    diasInput?.addEventListener('input', updatePreview);
    diasInput?.addEventListener('change', updatePreview);
