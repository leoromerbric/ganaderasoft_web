...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1">Información general y detalles de la cuenta de usuario (API V2)</p>
        </div>
        <a href="{{ route('dashboard') }}" class="inline-flex items-center text-sm text-ganaderasoft-azul hover:text-ganaderasoft-celeste font-medium transition-colors">
    ...
    <div class="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100">
        <!-- Header Banner -->
        <div class="h-32 bg-gradient-to-r from-ganaderasoft-celeste via-ganaderasoft-azul to-ganaderasoft-negro relative">
            <div class="absolute -bottom-10 left-8">
                <div class="w-24 h-24 rounded-2xl bg-white p-1 shadow-lg flex items-center justify-center border-2 border-white">
                    <div class="w-full h-full rounded-xl bg-gradient-to-br from-ganaderasoft-celeste to-ganaderasoft-azul flex items-center justify-center text-white text-3xl font-bold">
        ...
        <!-- Identity Section -->
        <div class="pt-14 pb-6 px-8 flex flex-col md:flex-row justify-between md:items-center gap-4 border-b border-gray-100">
            <div>
                <div class="flex items-center space-x-3">
                    <h2 class="text-2xl font-bold text-ganaderasoft-negro">{{ $user['name'] ?? 'Usuario' }}</h2>
                    <span class="px-3 py-1 text-xs font-semibold rounded-full {{ ($user['status'] ?? 'activo') === 'activo' ? 'bg-green-100 text-green-800 border border-green-200' : 'bg-red-100 text-red-800 border border-red-200' }}">
            ...
            <div class="flex flex-wrap gap-2">
                @if(!empty($user['roles']))
                    @foreach($user['roles'] as $role)
                        <span class="px-3 py-1.5 text-xs font-medium bg-ganaderasoft-celeste/15 text-ganaderasoft-azul border border-ganaderasoft-celeste/30 rounded-lg flex items-center space-x-1">
                ...
                <div class="space-y-3 text-sm">
                    @if(!empty($user['propietario']))
                        ...
                        <div class="flex justify-between py-1 border-b border-gray-200/60">
                            ...
                            <span class="font-medium text-gray-900">{{ ($persona['nombre'] ?? '') . ' ' . ($persona['apellido'] ?? '') }}</span>
                        </div>
                        <div class="flex justify-between py-1 border-b border-gray-200/60">
